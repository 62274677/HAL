...
import datetime
# import sympy
current_file_path = os.getcwd() + "/HAL/"
storage = ''
...
def image_writer(text):
    ...
    fnt = ImageFont.truetype(font=current_file_path +
                             'Hack-Regular.ttf', size=100, encoding="unic")
...
def read_sheet(sheet_url, type, worksheet_index):
    ...
    meetingURL_cell = sheet.get_worksheet(
        worksheet_index).find(sheet_content[0][0])
    fullsheet = sheet.get_worksheet(worksheet_index).get_all_values(
...
def generate_ccdc_calendar():
    ...
    zoom_links = [re.match('=hyperlink\("(.+)",".+"\)', link)
                  for link in links]
    zoom_links.pop(0)
    print(zoom_links)
...
def find_next_meeting(sheet):
    ...
    for i in range(2, len(sheet)-1):
        ...
        date = re.sub(r'(.+ \d{4}).*', '\g<1>',
                      re.sub(r'(\d+)(st|nd|rd|th)', '\g<1>', date))
        print(date)
        if date.lower() == 'tba':
            return sheet[i], i-1
