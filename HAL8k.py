...
imagefile = hal_commands.current_file_path+"image.png"
...
async def on_message(message):
    if message.content.startswith('$'):
        if (message.channel.id in hal_commands.storage["whitelist"]["channel"].values() or message.guild.name == 'Maximum Development' or message.author.id in hal_commands.storage["whitelist"]["user"]["admin"].values()):
            msg = ' '
            ...
            if content.upper() == 'HELP' or ('man' in content and 'HAL' in content):
                ...
                message_type = 'message'
                msg = "I'm sorry, {}, I'm afraid I can't do that.".format(
                    message.author.mention)

            elif content.startswith('ccdc calendar'):
                await message.add_reaction('👍')
                if content == 'ccdc calendar embed':
                    # message, topics, c,zlink, sheet_content_original
                    ...
                    ccdcEmbed = discord.Embed(
                        color=0xbf2c34, title='Upcoming CCDC Meeting (CLICK HERE FOR REMINDERS)', url=ccdc_calendar[2])
                    print(meeting)
                    print("\n")
                    ...
                    if(zlink):
                        ccdcEmbed.set_author(name="Click here for zoom link!", url=zlink,
                                             icon_url="https://upload.wikimedia.org/wikipedia/commons/9/9a/Gull_portrait_ca_usa.jpg")

                    else:
                        ccdcEmbed.set_author(name="This meeting will be on discord or zoom (no link yet).", url="https://google.com",
                                             icon_url="https://upload.wikimedia.org/wikipedia/commons/9/9a/Gull_portrait_ca_usa.jpg")

                    # ccdcEmbed.add_field(name=".", value=".", inline=False)
                    ccdcEmbed.add_field(
                        name=ccdc_calendar[1][meeting[1]], value=str("\a"), inline=False)
                    ccdcEmbed.add_field(
                        name="Next Meeting:", value=meeting[0][1], inline=False)
                    ccdcEmbed.add_field(
                        name="Meeting Time:", value=meeting[0][2], inline=False)
            ...
            elif content.startswith('whitelist'):
                hal_commands.storage["whitelist"]["channel"].update(
                    {message.channel.name + " (" + message.guild.name + ")": message.channel.id})
                json.dump(hal_commands.storage,
                          open(hal_commands.current_file_path + "storage.json", "w"))
                # print(emote(message.guild, "thumbsup")) #I should make this work later
                await message.add_reaction('👍')
                    ...
                    if msg_length > max_characters:
                        if msg.find('\n') < max_width:
                            ...
                            while i < msg_length-1:
                                ...
                                start = i
                                # make i the ceiling of max characters or message length
                                i += max_characters-1 if i + \
                                    (max_characters-1) <= msg_length-1 else msg_length-1-i
                            ...
                            msg = "```Error: Message exceeded {0} characters and was therefore split into multiple messages.```".format(
                                max_characters)
                        else:
                            hal_commands.image_writer(msg)

                            await message.channel.send(file=discord.File(imagefile))
                            os.remove("image.png")
                            msg = "Message exceeded {0} characters and {1} characters per line.".format(
